...
def write_mmap_file():
    ...
    index_data = [
        (source_id, min(group.index), max(group.index))
...
def benchmark(num_selects=100):
    ...
    print("Time per dataframe random id selection: {:.2} ms".format(
        1000 * (time.time() - start_time)))

    start_time = time.time()
    index_array = source_id_to_ranges_index.read()
    array = all_lc_data.read()
    print("Time for reading mmap data and index: {:.2} s".format(
    ...
    for _ in range(num_selects):
        ...
        assert isinstance(subsel, np.ndarray)
    print("Time per random id selection (mmap arrays): {} ms".format(
        1000 * (time.time() - start_time)))
